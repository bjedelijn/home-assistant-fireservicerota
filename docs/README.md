...
Current examples target **`1.1.0-rc.4`**.
...
The complete package combines these building blocks in one privacy-safe example. The RC4 example also demonstrates repeated TTS wording, separate day/night volumes, optional on-duty + Do Not Disturb gating for nighttime speech, and explicit Cast-session stop for speaker targets while leaving tablet targets untouched. Copy examples only after replacing placeholder entity IDs with the entities from your own Home Assistant installation.
